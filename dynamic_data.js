    ...
    try {
        ...
        projects.forEach(item => {
            if (item.category === 'personal' && projContainer) {
                // Project HTML 
                const projectHTML = `
                    <a href="${item.live_link || '#'}" class="glass-panel rounded-2xl overflow-hidden flex flex-col group cursor-pointer min-w-[320px] md:min-w-[400px] snap-center" target="_blank">
                        <div class="h-48 bg-gray-800 relative overflow-hidden shrink-0">
                            <img src="${item.image || 'https://placehold.co/600x400/9a3412/FFF?text=Project'}" class="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:scale-110 transition-transform duration-700">
                    ...
                    <div class="experience-item relative flex flex-col gap-8 items-center group">
                        ...
                        <div class="md:w-1/2 w-full pl-12 md:pl-0 flex exp-image opacity-80 group-hover:opacity-100 transition duration-500">
                            <div class="w-full max-w-md aspect-video bg-gray-800 rounded-xl overflow-hidden border border-white/10 shadow-2xl">
                                <img src="${item.image || 'https://placehold.co/600x400/be185d/FFF?text=Image'}" class="w-full h-full object-cover">
                            </div>
                        </div>
                    </div>
                `;
